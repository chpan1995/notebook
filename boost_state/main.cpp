...
// // 运行状态（默认进入 Moving）
// struct Operational2 : sc::simple_state<Operational2, Robot> {
...
// // 充电子状态：
// struct chargePause : sc::simple_state<chargePause, Charging> {
...
//     robot.process_event(EvMove()); // 切换到
...
struct PoweredOff : sc::simple_state<PoweredOff, Powered> {
    void entry() { std::cout << "进入电源关闭状态" << std::endl; }
    void exit() { std::cout << "离开电源关闭状态" << std::endl; }
};

// 电源开启状态 - 包含两个正交区域
struct PoweredOn : sc::simple_state<PoweredOn, Powered, mpl::list<PlaybackRegion, AudioRegion>> {
    typedef sc::transition<EvPowerOff, PoweredOff> reactions;
